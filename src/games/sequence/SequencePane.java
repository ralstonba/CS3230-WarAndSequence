...
public class SequencePane extends BorderPane {
	...
	if ( p.getType() == PieceType.BLUE )
	{
	    ...
	    pt.setOnFinished(e
			...
			PauseTransition ptt = new PauseTransition(Duration.millis(15));
			ptt.setOnFinished(ev -> changeParents(cardToDeal, player1Hand));
			ptt.play();
	    });
	} else
	{
	    ...
	    pt.setOnFinished(e
			...
			cardToDeal.setTranslateX(0);
			PauseTransition ptt = new PauseTransition(Duration.millis(15));
			ptt.setOnFinished(ev -> changeParents(cardToDeal, player2Hand));
			ptt.play();
	    });
	}
    ...
    private void changeParents(Card c, VBox v)
    {
	v.getChildren().add(c);
	c.setOnDragDetected((MouseEvent event)
		-> 
		{
		    if ( (bluePlayerTurn && ((VBox) c.getParent()) == player2Hand) || (!bluePlayerTurn && ((VBox) c.getParent()) == player1Hand) )
		    {
			return;
		    }

		    System.out.println("Drag detected on: " + c.toString());
		    Dragboard db = c.startDragAndDrop(TransferMode.MOVE);
		    ClipboardContent content = new ClipboardContent();
		    content.putImage(c.getImage());
		    db.setContent(content);
		    c.setVisible(false);
		    event.consume();
	});

	c.setOnDragDone((DragEvent event)
		-> 
		{
		    System.out.println("DragDone on: " + c.toString());
		    if ( event.getTransferMode() == TransferMode.MOVE )
		    {
			//Card was drop was successful
			((VBox) c.getParent()).getChildren().remove(c);
	...
	c.setOnMouseEntered(e
		-> 
		{
		    if ( bluePlayerTurn && (VBox) c.getParent() == player1Hand
			    || !bluePlayerTurn && (VBox) c.getParent() == player2Hand )
		    {
