...
public class SequencePane extends BorderPane {
    ...
    public Transition dealCard(Player p) {
        ...
        cardToDeal.setOnMouseEntered(e -> {
            if (bluePlayerTurn && (VBox) cardToDeal.getParent() == player1Hand || 
                    !bluePlayerTurn && (VBox) cardToDeal.getParent() == player2Hand) {
                ScaleTransition st = new ScaleTransition(Duration.millis(100), cardToDeal);
                st.setToX(1.5);
                st.setToY(1.5);
                st.play();
            }
        });
        cardToDeal.setOnMouseExited(e -> {
            ScaleTransition st = new ScaleTransition(Duration.millis(100), cardToDeal);
                st.setToX(1);
                st.setToY(1);
                st.play();
